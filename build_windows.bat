...
cd /d "%~dp0"
echo ========================================
echo TOMS SLA MN - Windows build
...
echo Dang cai cac thu vien cua TOMS SLA MN...
...
if errorlevel 1 goto :failed
echo Dang dong goi TOMS-SLA-MN.exe (file co the lon vi kem ca Chromium)...
python -m PyInstaller --noconfirm --clean --onefile --windowed --collect-all playwright --name TOMS-SLA-MN app.py
if errorlevel 1 goto :failed

if not exist "dist\TOMS-SLA-MN.exe" (
    echo [LOI] PyInstaller khong tao ra dist\TOMS-SLA-MN.exe
    goto :failed
)

echo Dang tao ma kiem tra SHA-256...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$hash=(Get-FileHash -Algorithm SHA256 -LiteralPath 'dist\TOMS-SLA-MN.exe').Hash.ToLower(); Set-Content -LiteralPath 'dist\TOMS-SLA-MN.exe.sha256' -Value ($hash + '  TOMS-SLA-MN.exe') -Encoding ascii"
if errorlevel 1 goto :failed
if not exist "dist\TOMS-SLA-MN.exe.sha256" (
...
echo %CD%\dist\TOMS-SLA-MN.exe
echo %CD%\dist\TOMS-SLA-MN.exe.sha256
echo Chi can chep file TOMS-SLA-MN.exe sang may Windows dich lan dau.
