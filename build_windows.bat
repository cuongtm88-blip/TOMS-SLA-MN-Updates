...
python -m PyInstaller --noconfirm --clean --onedir --windowed --collect-all playwright --name TOMS-SLA-MN app.py
if errorlevel 1 goto :failed

if not exist "dist\TOMS-SLA-MN\TOMS-SLA-MN.exe" (
    echo [LOI] PyInstaller khong tao ra ung dung onedir
    goto :failed
)

"dist\TOMS-SLA-MN\TOMS-SLA-MN.exe" --browser-self-test
if errorlevel 1 goto :failed
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$hash=(Get-FileHash -Algorithm SHA256 -LiteralPath 'dist-installer\TOMS-SLA-MN.exe').Hash.ToLower(); Set-Content -LiteralPath 'dist-installer\TOMS-SLA-MN.exe.sha256' -Value ($hash + '  TOMS-SLA-MN.exe') -Encoding ascii"
if errorlevel 1 goto :failed
if not exist "dist-installer\TOMS-SLA-MN.exe.sha256" (
...
echo %CD%\dist-installer\TOMS-SLA-MN.exe
echo %CD%\dist-installer\TOMS-SLA-MN.exe.sha256
echo Chi can chep va chay bo cai TOMS-SLA-MN.exe tren may Windows dich.
